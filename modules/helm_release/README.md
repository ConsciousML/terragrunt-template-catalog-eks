...
| <a name="input_helm_chart_version"></a> [helm\_chart\_version](#input\_helm\_chart\_version) | Version of the Helm chart to install. Omit for a local chart, its own Chart.yaml already pins one. | `string` | `null` | no |
...
| <a name="input_repository"></a> [repository](#input\_repository) | URL of the Helm chart repository. Omit for a local chart (chart set to a directory path). | `string` | `null` | no |
