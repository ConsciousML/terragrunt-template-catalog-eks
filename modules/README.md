...
This directory contains reusable Terraform modules for deploying IaC resources.

## What are Modules?

A **module** is a directory containing Terraform configuration files that create IaC resources.
...
- Define specific provider versions in `versions.tf`
- Exposes outputs in `outputs.tf` (when applicable)
- Configures providers in `providers.tf` (when applicable)

## How are Modules Used?

We're using Terragrunt in this repository: modules are not used directly as in native Terraform setups.
