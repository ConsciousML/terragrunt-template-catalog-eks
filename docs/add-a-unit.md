...
Map the module's inputs from `values`, the per-environment settings you set in the stack file, replacing each `<...>` placeholder with an input of the module:
```hcl
inputs = {
...
Describe the chart in `inputs`, replacing each `<...>` placeholder with your chart's details. The cluster name comes from the dependency the provider file added:
```hcl
inputs = {
...
Map your module's variables from `values`, replacing each `<...>` placeholder with one of them:
```hcl
inputs = {
...
Wire shared configuration into `inputs` from the `.hcl` files under `pipelines/`, listed in the [shared configuration reference](/docs/reference/shared_configuration/). Read what `root.hcl` already loads through `include.root.locals`, and any other file with `find_in_parent_folders`. Prefix resource names with the environment so they don't collide across environments:
```hcl
include "root" {
  path   = find_in_parent_folders("root.hcl")
  expose = true
...
inputs = {
  name = "${include.root.locals.environment}-<name>" # e.g. "${include.root.locals.environment}-loki-chunks"
  cidr = local.vpc_cidr
}
```

If your unit needs another unit's outputs, add a `dependency` block pointing at that unit's directory, and read its outputs in `inputs`. If it only has to run after the other unit, without reading its outputs, set `skip_outputs = true`. For example, the EBS CSI driver add-on reads the cluster name from `cluster`, and only has to run after `cilium_cep_restart`:
```hcl
# units/eks/addons/ebs_csi_driver/addon/terragrunt.hcl
...
## Wire It into the Dev Stack

Add a `unit` block to [`pipelines/dev/eks/stack/terragrunt.stack.hcl`](https://github.com/ConsciousML/terragrunt-template-catalog-eks/tree/main/pipelines/dev/eks/stack/terragrunt.stack.hcl). Point `source` at your unit, and set `path` to the unit's directory under `units/`. For example, for the Loki S3 chunks bucket:
```hcl
unit "loki_s3_chunks" {
...
If your unit wraps a registry module, pin the module's version as a `version_<name>` local at the top of the file, and pass it as `version`:
```hcl
locals {
  version_s3 = "5.15.1"
}
...
If your unit wraps an in-repo module, pass `local.version` instead, the catalog version the stack already resolves:
```hcl
...
If you wrote or changed an in-repo module, commit and push your branch first. Units fetch in-repo modules from git at your current branch, so local module changes aren't picked up until they're pushed:
```bash
...
Every unit needs a committed provider lock file, so a tag always resolves the same provider versions. CI fails on a unit without one.
...
This runs [`scripts/sync-lock-files.sh`](https://github.com/ConsciousML/terragrunt-template-catalog-eks/tree/main/scripts/sync-lock-files.sh), which copies each `.terragrunt-stack/<path>/.terraform.lock.hcl` to `units/<path>/.terraform.lock.hcl`. That's why the unit's `path` in the stack file should match its directory under `units/`.

Some units can't use the same path, for example when a stack instantiates one unit several times. The script then reports them as failed:
```text
...
If yours is one of them, copy its lock file into the unit's directory by hand, replacing each `<...>` placeholder:
```bash
...
Check that the new lock file landed in your unit's directory, then commit it:
```bash
git status units/
git add units/eks/addons/loki/s3/chunks/.terraform.lock.hcl
git commit -m "chore: pin provider lock files"
```

## Release It

Push your branch and open a pull request, replacing each `<...>` placeholder:
```bash
git push origin <your branch>
gh pr create --title "<title>" --body "<what the unit deploys, and how you validated it>"
```
