...
Username is `admin`. Retrieve the password from the chart-generated secret:
```bash
kubectl get secret kube-prometheus-stack-grafana -n monitoring \
  -o jsonpath='{.data.admin-password}' | base64 --decode
```
This secret is randomly generated by the chart on install. It doesn't yet follow the [ESO pattern used for ArgoCD's admin password](../units/eks/addons/argocd/README.md); that's a planned follow-up.
