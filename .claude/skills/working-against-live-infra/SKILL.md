...
5. Apply, picking the branch that matches the goal:
   - **Testing a specific unit**: from the stack directory, `terragrunt stack generate`, then
     ...
     `terragrunt apply -auto-approve`. A unit applied on its own without its dependencies already
     applied fails or plans against stale/mock state.
...
`terragrunt hcl validate --working-dir units/<...>` (or any other terragrunt command run straight
against a unit under `units/`) fails with a misleading `must specify a 'path' parameter` error
even when the config is correct. `include "root"` and stack-provided values only resolve inside a
generated stack. Validate through the normal flow instead: `terragrunt stack generate`, then
`terragrunt run --all plan` (or the targeted-unit apply flow above) from `.terragrunt-stack/<unit>`.

## Tearing down

Only destroy when the user asks for it.

- **Testing a specific unit**: `cd` into its `.terragrunt-stack/<unit>` directory and run
  `terragrunt destroy -auto-approve`. Destroy in reverse dependency order if you're tearing down
