...
Add an entry to `local.endpoint_host_offsets` in [`pipelines/vpc_endpoints.hcl`](../../../pipelines/vpc_endpoints.hcl), with an offset unused by any other entry. That offset is read by both this unit and [`../endpoint_cidrs`](../endpoint_cidrs/), which is what actually computes the pinned IP. That's enough to get the endpoint and its pinned IP created.
