...
Run via [`pipelines/bootstrap/tailscale`](../../pipelines/bootstrap/tailscale/) before deploying the Tailscale operator into any EKS cluster.

## What's Inside

- **[workflow_identity_federation](workflow_identity_federation/)**: Creates a Tailscale Workload Identity Federation credential scoped to a GitHub repository via OIDC. CI authenticates using short-lived tokens instead of a stored OAuth secret. The credential is tagged `tag:ci`, which the [ACL](../eks/addons/tailscale/acl/) grants the right to create auth keys for `tag:k8s-operator`, the permission the Tailscale operator needs to register itself when deployed into the cluster
- **[github_secrets](github_secrets/)**: Stores `workflow_identity_federation`'s credentials as GitHub secrets so CI workflows can authenticate to Tailscale at runtime
