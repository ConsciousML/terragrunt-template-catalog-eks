...
- **[addons/prometheus_stack](addons/prometheus_stack/README.md)**: Grafana admin password (Secrets Manager). The kube-prometheus-stack Helm release (Prometheus, Alertmanager, Grafana) and its HTTPRoutes are deployed through app-of-apps
...
- **[addons/argocd](addons/argocd/README.md)**: GitOps controller with admin password (Secrets Manager) synced in via ESO through app-of-apps
...
> **Note**: Karpenter's NodePool is capped at 10 vCPUs by default and provisions `spot` instances. Raise `spec.limits.cpu` or switch `karpenter.sh/capacity-type` to `on-demand` in [`helm-karpenter-config/templates/node-pool.yaml`](https://github.com/ConsciousML/argocd-app-of-apps-template/blob/main/helm-karpenter-config/templates/node-pool.yaml) in the App of Apps repository for production stability.
