...
- **[`units/eks/addons/loki`](../loki/)**: reads the chunks and ruler S3 bucket IDs to configure `loki`, and depends on `iam_role` so Loki's Pod Identity association exists before ArgoCD deploys it
